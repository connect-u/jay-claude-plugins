...
### 3.4 커밋 리듬 — VCS 위임의 마지막 마일 (2026-07-23 추가)

v0.1 §3은 정본의 이력·롤백·동기화를 git에 위임했지만 "누가 언제 커밋하는가"가 비어 있었다 — 위임의 마지막 마일. manifest 지침이 채운다: **지식이 사건 단위로 태어나므로, 작업 결과물도 같은 리듬으로 존재해야 한다.** 지식 탄생 시점 부근에서 작은 단위로 커밋하고, `.memorj` 변경은 그것을 낳은 작업과 함께 커밋한다.

근거: 세밀한 커밋 히스토리는 이 저장소의 **결과물 쪽 쌍둥이**다 — 엔트리를 읽는 미래 세션이 "이 결정이 만든 변화"를 대응하는 커밋에서 찾을 수 있어야 지식과 결과물의 계보가 연결된다. 별도 메커니즘(커밋 해시 참조 등)은 두지 않는다 — 단위가 정렬되면 시간축(epoch·ts)만으로 대응이 복원된다. 프로젝트가 `.memorj`를 gitignore하면(공유 repo의 개인 저장소) 지침은 자연히 무해하다. 커밋은 메인 세션의 일 — 서브에이전트 주입문에는 넣지 않는다.
